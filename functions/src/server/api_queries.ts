...
import {PriceTableEntries, EODApiResponse, EODAssetEntry} from "../types";
...
export const getAssetPrices = async (
  ...
  try {
    ...
    throw Error(`Couldn't fetch rates, because: ${err}`);
  }
};
...
const createCryptoMapFromCoinGeckoIDs = async (
    coinGeckoIDs: string[]
): Promise<{ [key: string]: { symbol: string, name: string } }> => {
  try {
    const response = await coinGeckoClient.coinList({});
    const cryptoMap: { [key: string]: { symbol: string, name: string } } = {};
...
export const printUpToDateCryptoAssets = async () => {
